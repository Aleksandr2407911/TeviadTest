...
from sqlalchemy import JSON, CheckConstraint, ForeignKey, Integer, MetaData, String
...
class Images(AbstractModel):
    ...
    tasks: Mapped["Tasks"] = relationship(
        "Tasks", back_populates="images", uselist=False
    )
...
class Faces(AbstractModel):
    ...
    age: Mapped[int] = mapped_column(
        CheckConstraint("age >= 1 AND age <= 200"), nullable=False
    )
